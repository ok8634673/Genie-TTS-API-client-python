...
class TTSClientGUI:
    def __init__(self, root):
        ...
        self.proxy_type = self.config.get('Proxy', 'proxy_type', fallback='http')
        self.proxy_host = self.config.get('Proxy', 'proxy_host', fallback='')
        self.proxy_port = self.config.get('Proxy', 'proxy_port', fallback='')
        self.proxy_username = self.config.get('Proxy', 'proxy_username', fallback='')
        self.proxy_password = self.config.get('Proxy', 'proxy_password', fallback='')
        
        # 路径模式配置（Windows/Linux）
        self.path_mode = self.config.get('Path', 'path_mode', fallback='auto')  # auto, windows, linux
    ...
    def create_default_config(self):
        ...
        # 路径模式配置
        self.config['Path'] = {
            'path_mode': 'auto'  # auto, windows, linux
        }
        self.config['Recent'] = {}
        self.save_config()
    ...
    def setup_tools_tab(self):
        ...
        # 添加路径模式配置
        path_frame = ttk.LabelFrame(self.tools_frame, text="路径模式配置", padding=10)
        path_frame.pack(fill='x', padx=10, pady=10)
        
        ttk.Label(path_frame, text="路径模式:").grid(row=0, column=0, sticky='w', padx=5, pady=5)
        
        # 路径模式单选按钮组
        self.path_mode_var = tk.StringVar(value=self.path_mode)
        path_options_frame = ttk.Frame(path_frame)
        path_options_frame.grid(row=0, column=1, sticky='w', padx=5, pady=5)
        
        ttk.Radiobutton(path_options_frame, text="自动检测", variable=self.path_mode_var, value="auto").pack(side='left', padx=5)
        ttk.Radiobutton(path_options_frame, text="Windows", variable=self.path_mode_var, value="windows").pack(side='left', padx=5)
        ttk.Radiobutton(path_options_frame, text="Linux", variable=self.path_mode_var, value="linux").pack(side='left', padx=5)
        
        ttk.Button(path_frame, text="保存设置", command=self.update_path_mode).grid(row=0, column=2, padx=5, pady=5)
    ...
    def setup_proxy_tab(self):
        # 代理设置框架
        proxy_frame = ttk.LabelFrame(self.proxy_frame, text="代理配置", padding=10)
        proxy_frame.pack(fill='x', padx=10, pady=5)
        
        # 使用代理复选框
        self.use_proxy_var = tk.BooleanVar(value=self.use_proxy)
        use_proxy_check = ttk.Checkbutton(proxy_frame, text="使用代理", variable=self.use_proxy_var, command=self.toggle_proxy_fields)
        use_proxy_check.grid(row=0, column=0, columnspan=3, sticky='w', padx=5, pady=5)
        
        # 代理类型选择
        ttk.Label(proxy_frame, text="代理类型:", state=tk.NORMAL if self.use_proxy else tk.DISABLED).grid(row=1, column=0, sticky='w', padx=5, pady=5)
        self.proxy_type_var = tk.StringVar(value=self.proxy_type)
        proxy_type_frame = ttk.Frame(proxy_frame)
        proxy_type_frame.grid(row=1, column=1, columnspan=2, sticky='w', padx=5, pady=5)
        
        # 水平单选框布局
        proxy_type_frame.columnconfigure(0, weight=1)
        proxy_type_frame.columnconfigure(1, weight=1)
        
        ttk.Radiobutton(proxy_type_frame, text="HTTP", variable=self.proxy_type_var, value="http", state=tk.NORMAL if self.use_proxy else tk.DISABLED).grid(row=0, column=0, sticky='w')
        ttk.Radiobutton(proxy_type_frame, text="SOCKS5", variable=self.proxy_type_var, value="sock5", state=tk.NORMAL if self.use_proxy else tk.DISABLED).grid(row=0, column=1, sticky='w')
        
        # 代理主机
        ttk.Label(proxy_frame, text="代理主机:", state=tk.NORMAL if self.use_proxy else tk.DISABLED).grid(row=2, column=0, sticky='w', padx=5, pady=5)
        self.proxy_host_entry = ttk.Entry(proxy_frame, width=30)
        self.proxy_host_entry.insert(0, self.proxy_host)
        self.proxy_host_entry['state'] = tk.NORMAL if self.use_proxy else tk.DISABLED
        self.proxy_host_entry.grid(row=2, column=1, columnspan=2, sticky='we', padx=5, pady=5)
        
        # 代理端口
        ttk.Label(proxy_frame, text="代理端口:", state=tk.NORMAL if self.use_proxy else tk.DISABLED).grid(row=3, column=0, sticky='w', padx=5, pady=5)
        self.proxy_port_entry = ttk.Entry(proxy_frame, width=10)
        self.proxy_port_entry.insert(0, self.proxy_port)
        self.proxy_port_entry['state'] = tk.NORMAL if self.use_proxy else tk.DISABLED
        self.proxy_port_entry.grid(row=3, column=1, sticky='w', padx=5, pady=5)
        
        # 认证设置框架
        auth_frame = ttk.LabelFrame(self.proxy_frame, text="代理认证（可选）", padding=10)
        auth_frame.pack(fill='x', padx=10, pady=5)
        
        # 用户名
        ttk.Label(auth_frame, text="用户名:", state=tk.NORMAL if self.use_proxy else tk.DISABLED).grid(row=0, column=0, sticky='w', padx=5, pady=5)
        self.proxy_username_entry = ttk.Entry(auth_frame, width=20)
        self.proxy_username_entry.insert(0, self.proxy_username)
        self.proxy_username_entry['state'] = tk.NORMAL if self.use_proxy else tk.DISABLED
        self.proxy_username_entry.grid(row=0, column=1, padx=5, pady=5)
        
        # 密码
        ttk.Label(auth_frame, text="密码:", state=tk.NORMAL if self.use_proxy else tk.DISABLED).grid(row=1, column=0, sticky='w', padx=5, pady=5)
        self.proxy_password_entry = ttk.Entry(auth_frame, width=20, show="*")
        self.proxy_password_entry.insert(0, self.proxy_password)
        self.proxy_password_entry['state'] = tk.NORMAL if self.use_proxy else tk.DISABLED
        self.proxy_password_entry.grid(row=1, column=1, padx=5, pady=5)
        
        # 保存按钮
        button_frame = ttk.Frame(self.proxy_frame)
        button_frame.pack(fill='x', padx=10, pady=10)
        
        ttk.Button(button_frame, text="保存设置", command=self.save_proxy_settings).pack(side='right', padx=5, pady=5)
        
        # 提示信息
        ttk.Label(self.proxy_frame, text="注意：更改代理设置后，将在下次API调用时生效。", foreground="#666666", font=('Arial', 9)).pack(padx=10, pady=5, anchor='w')
        
    def toggle_proxy_fields(self):
        """根据是否启用代理来启用/禁用代理设置字段"""
        ...
        for widget in self.proxy_frame.winfo_children():
            if isinstance(widget, ttk.LabelFrame):
                for child in widget.winfo_children():
                    if isinstance(child, (ttk.Label, ttk.Entry, ttk.Radiobutton)):
                        child['state'] = state
            elif isinstance(widget, ttk.Label) and "注意" in widget['text']:
                # 跳过提示标签
                pass
    ...
    def save_proxy_settings(self):
        ...
        try:
            ...
            proxy_username = self.proxy_username_entry.get()
            proxy_password = self.proxy_password_entry.get()
            
            # 验证必要字段（如果启用了代理）
            if use_proxy:
                if not proxy_host:
                    messagebox.showerror("错误", "代理主机不能为空")
                    return
                if not proxy_port or not proxy_port.isdigit():
                    messagebox.showerror("错误", "代理端口必须为数字")
                    return
                
            # 更新配置
            self.update_config('Proxy', 'use_proxy', str(use_proxy))
            self.update_config('Proxy', 'proxy_type', proxy_type)
            self.update_config('Proxy', 'proxy_host', proxy_host)
            self.update_config('Proxy', 'proxy_port', proxy_port)
            self.update_config('Proxy', 'proxy_username', proxy_username)
            self.update_config('Proxy', 'proxy_password', proxy_password)
            ...
            self.status_var.set(f"保存代理设置失败: {str(e)}")
    ...
    def update_path_mode(self):
        """更新路径模式设置"""
        new_path_mode = self.path_mode_var.get()
        self.path_mode = new_path_mode
        self.update_config('Path', 'path_mode', new_path_mode)
        
        # 显示提示信息
        if new_path_mode == 'auto':
            mode_name = "自动检测"
        elif new_path_mode == 'windows':
            mode_name = "Windows"
        else:
            mode_name = "Linux"
            
        messagebox.showinfo("成功", f"路径模式已更新为: {mode_name}")
    ...
    def generate_filename_from_text(self, text, character_name):
        ...
        print(f"[路径生成] 使用缓存目录: {current_cache_dir}")
        
        # 确保目录存在
        try:
            os.makedirs(current_cache_dir, exist_ok=True)
            print(f"[路径生成] 缓存目录: {current_cache_dir} 已确认存在")
            
            # 检查目录是否可写
            if not os.access(current_cache_dir, os.W_OK):
                print(f"[路径生成] 警告: 缓存目录 {current_cache_dir} 没有写入权限")
                # 尝试使用临时目录作为备选
                import tempfile
                temp_dir = tempfile.gettempdir()
                print(f"[路径生成] 切换到临时目录: {temp_dir}")
                current_cache_dir = temp_dir
                os.makedirs(current_cache_dir, exist_ok=True)
        except Exception as e:
            print(f"[路径生成] 创建缓存目录失败: {e}")
            # 使用临时目录作为备选
            import tempfile
            temp_dir = tempfile.gettempdir()
            print(f"[路径生成] 强制使用临时目录: {temp_dir}")
            current_cache_dir = temp_dir
        
        # 根据路径模式处理路径分隔符
        final_path = ""
        if self.path_mode == 'auto':
            # 自动模式下使用系统默认路径分隔符
            final_path = os.path.normpath(os.path.join(current_cache_dir, filename))
        elif self.path_mode == 'windows':
            # Windows模式强制使用反斜杠
            final_path = os.path.join(current_cache_dir, filename).replace('/', '\\')
        else:  # linux
            # Linux模式强制使用正斜杠
            final_path = os.path.join(current_cache_dir, filename).replace('\\', '/')
        
        print(f"[路径生成] 最终生成的文件路径: {final_path}")
        return final_path
    ...
    def api_call(self, endpoint, data=None):
        ...
        try:
            ...
            if self.use_proxy and self.proxy_host and self.proxy_port:
                ...
                print(f"[API调用] 使用代理: {proxy_url} (类型: {proxy_type})")
            else:
                proxies = None
            ...
            if data:
                response = session.post(url, json=data, headers=headers, timeout=30, proxies=proxies)
            else:
                response = session.post(url, headers=headers, timeout=30, proxies=proxies)
        ...
        if not save_path:
            ...
            try:
                ...
                if self.path_mode == 'auto':
                    ...
                    # Windows模式强制使用反斜杠
                    cache_file_path = os.path.join(target_dir, filename).replace('/', '\\')
                else:  # linux
                    # Linux模式强制使用正斜杠
                    cache_file_path = os.path.join(target_dir, filename).replace('\\', '/')
    ...
    def _tts_thread(self, data, cache_file_path):
        ...
        if getattr(self, 'proxy_mode', False) and getattr(self, 'server_running', False):
            ...
            # 直接模式下确保目录存在
            if success and cache_file_path:
                try:
                    # 确保目标目录存在
                    os.makedirs(os.path.dirname(cache_file_path), exist_ok=True)
                    print(f"[直接模式] 确保目录存在: {os.path.dirname(cache_file_path)}")
                    
                    # 如果API返回的是二进制音频数据，直接保存
                    if isinstance(result, bytes) and len(result) > 0:
                        with open(cache_file_path, 'wb') as f:
                            f.write(result)
                        print(f"[直接模式] 音频数据已直接保存到: {cache_file_path}")
                except Exception as e:
                    print(f"[直接模式] 准备文件保存时出错: {e}")
        ...
        if success:
            ...
            if cache_file_path and os.path.exists(cache_file_path) and os.path.getsize(cache_file_path) > 0:
                ...
                print(f"[TTS完成] 文件已成功生成: {cache_file_path}, 大小: {file_size} 字节")
                messagebox.showinfo("成功", f"TTS转换完成，文件已保存: {cache_file_path}")
            else:
                print(f"[TTS完成] 文件未找到或为空: {cache_file_path}")
                print(f"[TTS完成] API调用结果: {result}")
                # 更详细的错误提示
                error_msg = "TTS转换完成，但未找到生成的音频文件。\n"
                error_msg += f"检查路径: {cache_file_path}\n"
                error_msg += "可能的原因：\n"
                error_msg += "1. 服务端未正确保存文件\n"
                error_msg += "2. 路径权限问题\n"
                error_msg += "3. 服务端返回的不是音频数据"
    ...
    def _speak_with_proxy_mode(self, data, cache_file_path):
        ...
        try:
            ...
            response = session.post(tts_url, json=data, timeout=30)
    ...
    def register_with_master(self, task_id):
        ...
        try:
            ...
            if self.use_proxy and self.proxy_host and self.proxy_port:
                ...
                session.proxies = proxies
            r = session.post(register_url, json=payload, timeout=5)
    ...
    def _test_connection_thread(self):
        try:
            ...
            # 尝试访问API根路径或健康检查端点
            response = requests.get(f"{self.upstream_api_url}/", timeout=5, proxies=proxies)
            if response.status_code == 200:
                messagebox.showinfo("连接测试", "连接成功!")
            else:
                # 如果根路径不行，尝试/docs路径（FastAPI的文档）
                response = requests.get(f"{self.upstream_api_url}/docs", timeout=5, proxies=proxies)
                if response.status_code == 200:
